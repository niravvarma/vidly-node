...
router.post("/", auth, async (req, res) => {
  ...
  // we cannot do the following because rental.save and movie.save both should be either be successful
  // or failed together in order to be in snync which is obvious.
  ...
  try {
    new fawn.Task()
      ...
      .run(); //new collection is created called as "ojlinttaskcollections" so that it keeps
    // track of intermediate step of what needs to be saved in mongodb. After the collection
    // is saved, it deletes the data from the collection. It is something like keeping temp data.
